...
makejobpostingcsv(config['csv_path'],config['search_keyword'],config['jsearch_api'],config['csv_name'],config['ollama_model_csvmaker'])
docs=load_jobs(os.path.join(config['csv_path'], config['csv_name']))
vector_db=makevectordb(docs,config['Huggingface_embedder_model'])
response=get_answer(config['ollama_model_responder'],vector_db,question)
print(response)
